...
void test_roots() {
   ...
   // In addition, the m₀(π) = 0, and m₀ is 2π periodic.
   // Recalling 𝓕[ψ](ω) = exp(iω/2)m₀(ω/2 + π)^{*}𝓕[𝜙](ω)*phase,
...
void test_wavelet_quadrature() {
   ...
   double domega = 1/8.0;
   for (double omega = 0; omega < 1; omega += domega) {
      // I suspect the quadrature is less accurate than special function evaluation, so this is just a sanity check:
      auto f = [&](double t) {
      ...
      std::cout << "Expected: |𝓕[ψ](" << omega << ")| = " << std::abs(expected) << "\n";
      std::cout << "Computed: |𝓕[ψ](" << omega << ")| = " << std::abs(computed) << "\n";
      std::cout << "arg(expected) = " << std::arg(expected) << "\n";
      std::cout << "arg(computed) = " << std::arg(computed) << "\n\n";
      if(!CHECK_MOLLIFIED_CLOSE(std::abs(expected), std::abs(computed), 1e-4)) {
         std::cerr << "  |𝓕[ψ](" << omega << ")| is incorrect.\n";
      }
      /*if(!CHECK_MOLLIFIED_CLOSE(std::arg(expected), std::arg(computed), 1e-4)) {
